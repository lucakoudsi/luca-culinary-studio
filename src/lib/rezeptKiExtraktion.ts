...
const KOMPONENTEN_ABLESEN = `Komponenten (nur bei mehrteiligen Gerichten): Erkennst du mehrere eigenständige Elemente, die JEWEILS GETRENNT zubereitet und erst beim Anrichten zusammengeführt werden (z.B. "Für die Sauce: ... Für den Belag: ..." oder klar getrennte Teile wie Basis/Füllung/Topping/Sauce -- Beispiel: ein Gericht aus "Carbonara Ball", "Tonnarelli" und "Pecorino-Espuma" hat drei Komponenten, weil jedes Element eigenständig zubereitet wird), trage sie strukturiert in "komponenten" ein (je ein Objekt mit "name", "zutaten", "zubereitung"). In diesem Fall bleiben die flachen Felder "zutaten" und "schritte" leere Arrays -- alles gehört dann in die jeweilige Komponente, nicht doppelt gepflegt.

Bei einem normalen, einteiligen Gericht (die meisten Fälle -- z.B. eine Kräuterbutter mit mehreren Zutaten und einer Zubereitung) bleibt "komponenten" ein leeres Array und ALLE Zutaten/Schritte stehen flach in "zutaten"/"schritte". GENAU EINE Komponente ist fast immer falsch: entweder gibt es wirklich mehrere getrennt zubereitete Elemente (dann mindestens zwei Einträge in "komponenten"), oder es ist ein einziges Element (dann "komponenten" leer, alles flach). Kommst du bei deiner Analyse nur auf einen einzigen Komponenten-Kandidaten, ist das ein Signal, dass es in Wahrheit ein einteiliges Gericht ist -- korrigiere dann auf die flachen Felder.

Orientiere dich an der TATSÄCHLICHEN Struktur des Gerichts, nicht an der Beschriftung der Vorlage: Manche Rezeptquellen überschreiben ihre schlichte, einteilige Zutatenliste selbst mit Wörtern wie "Die Komponenten" oder "Zutaten der Komponente", obwohl es sich um eine einzige, gemeinsam zubereitete Liste handelt. Lass dich davon NICHT zu einer einzelnen Komponente verleiten -- prüfe anhand der Zubereitung, ob wirklich mehrere separat hergestellte Teile vorliegen, unabhängig davon, wie die Quelle das selbst nennt.`;
