...
import { Search, Leaf, X, Thermometer, MapPin, Tag } from 'lucide-react';

const seasonOptions = ['Alle', 'Frühling', 'Sommer', 'Herbst', 'Winter', 'Ganzjährig'];
const categoryOptions: string[] = [
  'Alle', 'Gemüse', 'Obst', 'Fleisch', 'Fisch & Meeresfrüchte', 'Pilze',
  'Kräuter & Gewürze', 'Nüsse & Samen', 'Milchprodukte & Käse',
  'Getreide & Hülsenfrüchte', 'Öle & Fette', 'Fermentiertes',
];
...
type FlavorKey = typeof flavorKeys[number];
    ...
    <div className="flex items-center gap-2.5">
      ...
      <div className="flex-1 h-1.5 bg-background rounded-full overflow-hidden">
        <div className="h-full rounded-full transition-all duration-500"
          style={{ width: `${((value ?? 0) / 5) * 100}%`, background: color }} />
      </div>
      <span className="text-[11px] font-semibold w-4 text-right" style={{ color }}>{value ?? 0}</span>
    </div>
  );
}

function IngredientCard({ ingredient, onClick }: { ingredient: Ingredient; onClick: () => void }) {
  const color = catColor[ingredient.kategorie] || '#C9A84C';
  const geschmack = ingredient.geschmack ?? {};
  const topFlavor = flavorKeys.reduce<FlavorKey>((a, b) => (geschmack[a] ?? 0) >= (geschmack[b] ?? 0) ? a : b, 'umami');
  const topFlavorIdx = flavorKeys.indexOf(topFlavor);
  const aromas = Array.isArray(ingredient.aromaprofil) ? ingredient.aromaprofil : [];
  const saison = Array.isArray(ingredient.saison) ? ingredient.saison : [];
...
function IngredientDetail({ ingredient, onClose }: { ingredient: Ingredient; onClose: () => void }) {
  ...
  const aromas = Array.isArray(ingredient.aromaprofil) ? ingredient.aromaprofil : [];
  const saison = Array.isArray(ingredient.saison) ? ingredient.saison : [];
  const pairings = Array.isArray(ingredient.pairings) ? ingredient.pairings : [];
      ...
      <div className="bg-surface border border-border-strong rounded-2xl w-full max-w-xl max-h-[88vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="p-7">
          <div className="flex items-start justify-between mb-5">
            <div>
              <div className="text-[11px] text-gold font-semibold uppercase tracking-wider mb-1">{ingredient.kategorie}</div>
              ...
              <div className="flex items-center gap-2 mt-1.5 text-[12px] text-text-muted">
                <MapPin size={11} />{ingredient.herkunft}
                <span className="mx-1">·</span>
                <Thermometer size={11} />{ingredient.lagertemp}
              </div>
            </div>
            <button onClick={onClose} className="text-text-secondary hover:text-text-primary p-1"><X size={20} /></button>
          </div>

          <p className="text-[13px] text-text-secondary leading-relaxed mb-6">{ingredient.beschreibung}</p>
            ...
            <div className="flex gap-2">
              {saison.map(s => (
                <span key={s} className="text-[12px] px-3 py-1.5 rounded-lg font-semibold"
                  style={{ background: `${seasonColors[s] ?? '#A89880'}18`, color: seasonColors[s] ?? '#A89880', border: `1px solid ${seasonColors[s] ?? '#A89880'}35` }}>
                  {s}
                </span>
              ))}
            </div>
          ...
          <div className="mb-5 bg-card rounded-xl p-4">
            ...
            <div className="space-y-3">
              {flavorKeys.map((key, i) => (
                <FlavorBar key={key} label={flavorLabels[i]} value={geschmack[key] ?? 0} color={flavorColors[i]} />
              ))}
            </div>
          </div>
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      <div className="px-4 sm:px-8 pt-6 sm:pt-8 pb-6" style={{ borderBottom: '1px solid var(--border)' }}>
        ...
        <h1 className="font-heading font-bold leading-none" style={{ fontSize: 28, color: 'var(--text)', letterSpacing: '2px', textTransform: 'uppercase' }}>Zutatenbibliothek</h1>
        <p className="mt-1.5" style={{ color: 'var(--text-muted)', fontSize: 13 }}>{filtered.length} Zutaten · Klicke für Aromaprofil & Pairings</p>
      </div>
